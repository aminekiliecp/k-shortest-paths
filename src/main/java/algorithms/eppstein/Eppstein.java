package algorithms.eppstein;

import algorithms.KSPAlgorithm;
import tools.Edge;
import tools.Graph;
import tools.Node;
...
import util.Path;
import util.ShortestPathTree;

import java.util.*;

/**
 * Eppstein.Eppstein's algorithm for computing the K shortest paths between two nodes in a graph.
 *
...
public class Eppstein implements KSPAlgorithm {
    ...
    /**
     * Computes the K shortest paths (allowing cycles) in a graph from node s to node t in graph G using Eppstein.Eppstein's
     * algorithm. ("Finding the k Shortest Paths", Eppstein.Eppstein)
     *
     * Some explanatory notes about how Eppstein.Eppstein's algorithm works:
     ...
     * - Eppstein.Eppstein's algorithm is basically a scheme for re-organizing this heap so that each heap node has at most 4
    ...
    public List<Path> ksp(Graph graph, String sourceLabel, String targetLabel, int K) {
        ...
        // Create a virtual/dummy heap that is the root of the overall Eppstein.Eppstein heap. It represents the best path from
        ...
        for (int i = 0; i < K && pathPQ.size() > 0; i++) {
            ...
            // Push the (up to 3) children of this path within the Eppstein.Eppstein heap onto the priority queue
            ...
            /* Check for the existence of a potential fourth child, known as a "cross edge", to push onto the queue.
                This heap edge/child does not need to be explicitly represented in the Eppstein.Eppstein heap because it is easy
    ...
    /**
     * Push the (up to 3) children (within the Eppstein.Eppstein heap) of the given (kth) path, onto the priority queue.
     *
    ...
    protected static void recursiveOutrootHeaps(String nodeLabel, EppsteinArrayHeap currentArrayHeap, HashMap<String, EppsteinHeap> nodeHeaps, HashMap<String, EppsteinHeap> outrootHeaps, Graph reversedSPT) {
        ...
        if (sidetrackHeap != null) {
            ...
            // The goal of Eppstein.Eppstein's algorithm is to re-use heap structures where possible
        ...
        // Convert from an array representation of the heap (Eppstein.EppsteinArrayHeap), which is convenient for accessing and
        // manipulating a binary heap, to a pointer representation (Eppstein.EppsteinHeap), which is consistent with the overall
...
class EppsteinHeap {
    ...
    private ArrayList<EppsteinHeap> children; // supports N children but Eppstein.Eppstein is limited to 4
...
/**
 * An array representation of a binary heap with additional functions specialized for implementing Eppstein.Eppstein's algorithm.
 */
 ...
 * paths during the execution of Eppstein.Eppstein's algorithm.
 */
