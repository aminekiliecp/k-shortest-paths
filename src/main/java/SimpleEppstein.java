...
/**
 * This is "simplified Eppstein.Eppstein's algorithm", a version of Eppstein.Eppstein's algorithm for computing the K shortest paths
 * between two nodes in a graph, which has been simplified by Brandon Smock.
 *
 * This simplified version eliminates the pre-processing and additional storage required by Eppstein.Eppstein's
 * algorithm. It is faster for smaller values of K due to the reduction in pre-processing, but it incurs more
 * computation per path, so for large values of K, Eppstein.Eppstein's algorithm is faster.
 *
 * It is primarily intended to be a useful first step for understanding how Eppstein.Eppstein's algorithm works and how it can be
 * implemented.
 *
...
public class SimpleEppstein implements KSPAlgorithm {
     ...
     * version of Eppstein.Eppstein's algorithm. ("Finding the k Shortest Paths", Eppstein.Eppstein)
     *
     ...
     * version of Eppstein.Eppstein's algorithm. ("Finding the k Shortest Paths", Eppstein.Eppstein)
     *
     * See Eppstein.Eppstein.java for some explanatory notes about how Eppstein.Eppstein's algorithm works.
     *
     * - In this simplified version, like Eppstein.Eppstein's algorithm we represent each path using its unique sequence of
     * sidetrack edges.
     * - Like Eppstein.Eppstein's algorithm, there is a min heap which partially orders these paths, and we can use this fact
     * to efficiently search the space of paths to find the K shortest.
     * - Like Eppstein.Eppstein's algorithm, we never fully generate this heap, and we can use a second heap to store the
     * portions of the first heap that we have traversed/generated.
     *
     * DIFFERENCES IN THIS SIMPLIFIED VERSION:
     * - The first heap has O(|E|) children per node, and one of the big contributions of Eppstein.Eppstein was a scheme for
     ...
     * - Thus for small to moderate values of K, this simplified algorithm ought to be faster than Eppstein.Eppstein's algorithm,
     * but for large values of K, Eppstein.Eppstein's algorithm will eventually overtake this algorithm in efficiency.
     *
     * @param graph         the graph on which to compute the K shortest paths from s to t
     * @param sourceLabel   the starting node for all of the paths
     * @param targetLabel   the ending node for all of the paths
 ...
 * paths during the execution of the simplified version of Eppstein.Eppstein's algorithm.
 */
